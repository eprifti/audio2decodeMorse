#!/usr/bin/env bash
# End-to-end helper for the multi-task prototype (CTC + bit/gap heads).
# Usage: ./launch_run_multitask.sh [run_name]
set -euo pipefail
...
RUN_NAME="${1:-prototype_multitask_$(date +%Y%m%d_%H%M%S)}"
GEN_CFG="config/generation.yaml"
TRAIN_CFG="config/prototype_multitask.yaml"
RUN_DIR="outputs/${RUN_NAME}"

# Skip regeneration if manifests already exist unless FORCE_REGEN=1
