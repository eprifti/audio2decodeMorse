#!/usr/bin/env bash
# End-to-end helper for the multitask counts prototype (CTC + bit/gap + counts).
# Usage: ./launch_run_multitask_counts.sh [run_name]
set -euo pipefail
...
RUN_NAME="${1:-prototype_multitask_counts_$(date +%Y%m%d_%H%M%S)}"
GEN_CFG="config/generation.yaml"
TRAIN_CFG="config/prototype_multitask_counts.yaml"
